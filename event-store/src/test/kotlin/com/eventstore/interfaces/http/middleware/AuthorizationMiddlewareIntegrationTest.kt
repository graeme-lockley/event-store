...
class AuthorizationMiddlewareIntegrationTest {
    ...
    @Test
    fun `allows access to public endpoints without authentication`() =
        testApplication {
            setupApplication {
                get("/health") {
    ...
    @Test
    fun `returns 401 when no session provided for protected endpoint`() =
        testApplication {
            setupApplication {
    ...
    @Test
    fun `returns 401 when invalid session provided`() =
        testApplication {
            setupApplication {
                get("/tenants/$testTenantName") {
                    call.respond(HttpStatusCode.OK, mapOf("name" to testTenantName))
            ...
            val response =
                client.get("/tenants/$testTenantName") {
    ...
    @Test
    fun `returns 403 when permission denied`() =
        testApplication {
            setupApplication {
                get("/tenants/$testTenantName") {
                    call.respond(HttpStatusCode.OK, mapOf("name" to testTenantName))
    ...
    @Test
    fun `allows access when permission granted`() =
        testApplication {
            // Grant permission
            runBlocking {
                val grantEvent =
    ...
    @Test
    fun `extracts tenant context from URL`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
    ...
    @Test
    fun `extracts namespace context from URL`() =
        testApplication {
            ...
            runBlocking {
                ...
                val namespaceEvent =
                    Event(
                        ...
                        payload =
                            com.eventstore.domain.events.NamespaceCreatedEvent(
            ...
            val response =
                client.get("/tenants/$testTenantName/namespaces/$testNamespaceName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.OK, response.status)
        }
    ...
    @Test
    fun `maps HTTP methods to correct permissions`() =
        testApplication {
            runBlocking {
            ...
            val putResponse =
                client.put("/tenants/$testTenantName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
    ...
    @Test
    fun `allows ADMIN permission to access all operations`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
            ...
            val deleteResponse =
                client.delete("/tenants/$testTenantName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
    ...
    @Test
    fun `allows access with session cookie`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
    ...
    @Test
    fun `handles URLs with trailing slashes`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
    ...
    fun `handles URLs with query parameters`() =
        testApplication {
            ...
            val response =
                client.get("/tenants/$testTenantName?filter=active&limit=10") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.OK, response.status)
        }
    ...
    @Test
    fun `handles global permissions for all resources in a tenant`() =
        testApplication {
                    ...
                    Event(
                        ...
                        payload =
                            PermissionGrantedEvent(
                                ...
                                principalType = PrincipalType.USER,
                                resourceType = ResourceType.TENANT,
                                // null = all tenant resources of this type in the tenant context
                                resourceId = null,
                                tenantResourceId = testTenantResourceId.toString(),
                                permissions = setOf(Permission.READ),
                                grantedBy = "admin",
    ...
    @Test
    fun `handles multiple permissions granted`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
    ...
    @Test
    fun `handles unsupported HTTP methods`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
            ...
            assertEquals(
                HttpStatusCode.OK,
                client.patch("/tenants/$testTenantName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }.status,
            )
        }
    ...
    @Test
    fun `handles Authorization header taking precedence over cookie`() =
        testApplication {
    ...
    @Test
    fun `handles empty Bearer token`() =
        testApplication {
            setupApplication {
                get("/tenants/$testTenantName") {
                    call.respond(HttpStatusCode.OK, mapOf("name" to testTenantName))
    ...
    @Test
    fun `handles malformed Authorization header`() =
        testApplication {
            setupApplication {
                get("/tenants/$testTenantName") {
                    call.respond(HttpStatusCode.OK, mapOf("name" to testTenantName))
    ...
    @Test
    fun `handles non-existent tenant gracefully`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
            ...
            val response =
                client.get("/tenants/non-existent-tenant") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
    ...
    @Test
    fun `handles topic routes with schema management permission`() =
        testApplication {
            val namespaceResourceId = UUID.randomUUID()
    ...
    @Test
    fun `handles LIST permission for collection endpoints`() =
        testApplication {
            runBlocking {
                val grantEvent =
                    Event(
    ...
    @Test
    fun `handles CREATE permission for collection endpoints`() =
        testApplication {
            runBlocking {
                    ...
                    Event(
                        ...
                        payload =
                            PermissionGrantedEvent(
                                ...
                                // All tenants
                                resourceId = null,
                                // Use system tenant for tenant creation
    ...
    @Test
    fun `handles event routes correctly`() =
        testApplication {
            val namespaceResourceId = UUID.randomUUID()
            runBlocking {
                // Create namespace
            ...
            val response =
                client.post("/tenants/$testTenantName/namespaces/$testNamespaceName/events") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.Created, response.status)
        }
    ...
    @Test
    fun `handles user routes correctly`() =
        testApplication {
            ...
            val response =
                client.get("/tenants/$testTenantName/users/$userResourceId") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.OK, response.status)
        }
    ...
    @Test
    fun `handles topic extraction from URL`() =
        testApplication {
            val namespaceResourceId = UUID.randomUUID()
            val topicResourceId = UUID.randomUUID()
            runBlocking {
            ...
            val response =
                client.get("/tenants/$testTenantName/namespaces/$testNamespaceName/topics/$testTopicName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.OK, response.status)
        }
    ...
    @Test
    fun `handles permission denied for specific resource when global permission exists`() =
        testApplication {
            ...
            val response2 =
                client.get("/tenants/$otherTenantName") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
    ...
    @Test
    fun `handles system-level endpoints without tenant context`() =
        testApplication {
            setupApplication {
    ...
    @Test
    fun `handles expired permissions`() =
        testApplication {
            runBlocking {
    ...
    @Test
    fun `handles URLs with special characters`() =
        testApplication {
            ...
            val response =
                client.get("/tenants/$tenantNameWithSpecialChars") {
                    header(HttpHeaders.Authorization, "Bearer $testSessionId")
                }
            assertEquals(HttpStatusCode.OK, response.status)
        }
