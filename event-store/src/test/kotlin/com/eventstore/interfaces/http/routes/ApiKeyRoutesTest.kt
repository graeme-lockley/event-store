...
class ApiKeyRoutesTest {
    ...
    private suspend fun grantUserPermissions(
        userId: String,
        tenantId: String,
    ) {
        ...
        application.grantPermission(
            GrantPermissionRequest(
                ...
                // For USER resource type, resourceId is the userId
                resourceId = userId,
        ...
        application {
            ...
            routing {
                ...
                val authenticationService =
                    com.eventstore.domain.services.auth.AuthenticationService(
                        domainApp.userProjectionService,
                        sessionManager,
                    )
                val authorizationService =
                    com.eventstore.domain.services.auth.AuthorizationService(
    ...
    @Test
    fun `POST creates API key successfully`() =
        testApplication {
            ...
            val response =
                client.post("/tenants/$tenantId/users/$userId1/api-keys") {
    ...
    @Test
    fun `GET list returns user's API keys`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.get("/tenants/$tenantId/users/$userId1/api-keys") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    fun `GET by ID returns API key`() =
        testApplication {
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.get("/tenants/$tenantId/users/$userId1/api-keys/$apiKeyId") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `GET by ID rejects access to other user's API key`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.get("/tenants/$tenantId/users/$userId2/api-keys/$apiKeyId") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    fun `DELETE revokes API key`() =
        testApplication {
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.delete("/tenants/$tenantId/users/$userId1/api-keys/$apiKeyId") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `DELETE rejects revoking other user's API key`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            grantUserPermissions(userId2, tenantId)
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.delete("/tenants/$tenantId/users/$userId2/api-keys/$apiKeyId") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `works with API key authentication`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
    ...
    @Test
    fun `POST rejects whitespace-only name`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `POST rejects name exceeding max length`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val body =
                CreateApiKeyRequestDto(
                    // Exceeds MAX_NAME_LENGTH of 255
                    name = "a".repeat(256),
                    description = "Test description",
                )
    ...
    @Test
    fun `POST rejects expiresAt in the past`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `POST rejects description exceeding max length`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val body =
                CreateApiKeyRequestDto(
                    name = "Test Key",
                    // Exceeds MAX_DESCRIPTION_LENGTH of 1000
    ...
    @Test
    fun `POST rejects missing userId parameter`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val response =
                client.post("/tenants/$tenantId/users//api-keys") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                    contentType(ContentType.Application.Json)
                    setBody(body)
                }
    ...
    @Test
    fun `POST creates API key with scopes`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `POST creates API key with expiration date`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `POST trims whitespace from name`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `GET list returns empty list when user has no keys`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `GET list includes revoked and expired keys`() =
        testApplication {
    ...
    @Test
    fun `GET list rejects missing userId parameter`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            // Use empty userId in URL - Ktor routing returns 404 for malformed paths
            val response =
                client.get("/tenants/$tenantId/users//api-keys") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `GET by ID returns revoked API key metadata`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val (apiKey, _) = application.createApiKey(userId1, "Test Key")
            val apiKeyId = apiKey.id
    ...
    @Test
    fun `GET by ID returns expired API key metadata`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val expiredDate = Instant.now().minusSeconds(3600)
    ...
    @Test
    fun `GET by ID returns 404 for non-existent key`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.get("/tenants/$tenantId/users/$userId1/api-keys/$nonExistentKeyId") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `GET by ID rejects missing keyId parameter`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `DELETE returns 409 for already revoked key`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val (apiKey, _) = application.createApiKey(userId1, "Test Key")
            val apiKeyId = apiKey.id
    ...
    @Test
    fun `DELETE uses consistent response format`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val (apiKey, _) = application.createApiKey(userId1, "Test Key")
            val apiKeyId = apiKey.id
    ...
    @Test
    fun `DELETE rejects missing keyId parameter`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val session = sessionManager.createSession(userId1)
            val response =
                client.delete("/tenants/$tenantId/users/$userId1/api-keys/") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                }
    ...
    @Test
    fun `DELETE can revoke expired key`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val expiredDate = Instant.now().minusSeconds(3600)
            val (apiKey, _) = application.createApiKey(userId1, "Expired Key", expiresAt = expiredDate)
    ...
    @Test
    fun `rejects revoked API key for authentication`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val (apiKey, plainKey) = application.createApiKey(userId1, "Auth Key")
    ...
    @Test
    fun `rejects expired API key for authentication`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            val expiredDate = Instant.now().minusSeconds(3600)
    ...
    @Test
    fun `rejects invalid API key format`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
    ...
    @Test
    fun `POST rejects user not found`() =
        testApplication {
            grantUserPermissions(userId1, tenantId)
            setupApplication {}
            ...
            val response =
                client.post("/tenants/$tenantId/users/$nonExistentUserId/api-keys") {
                    header(HttpHeaders.Authorization, "Bearer ${session.id}")
                    contentType(ContentType.Application.Json)
                    setBody(body)
                }
