...
class GetApiKeyServiceTest {
    ...
    @Test
    fun `returns null for non-existent API key`() = runTest {
        val retrieved = application.getApiKey("non-existent")
        assertNull(retrieved)
    }

    @Test
    fun `gets API key by ID with all fields`() = runTest {
    ...
    @Test
    fun `gets API keys by user ID`() = runTest {
        val user = application.createUser(
    ...
    @Test
    fun `returns empty list for user with no API keys`() = runTest {
        val user = application.createUser(
    ...
    @Test
    fun `returns empty list for non-existent user`() = runTest {
        val userKeys = application.getApiKeysByUserId("non-existent-user-id")
        assertTrue(userKeys.isEmpty())
    }

    @Test
    fun `gets API keys for multiple users`() = runTest {
