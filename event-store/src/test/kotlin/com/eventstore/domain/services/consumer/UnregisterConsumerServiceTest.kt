...
class UnregisterConsumerServiceTest {
    ...
    @Test
    fun `should unregister consumer successfully`() =
        runTest {
            val request =
    ...
    @Test
    fun `should throw exception when consumer not found`() =
        runTest {
    ...
    @Test
    fun `should throw exception when unregistering already unregistered consumer`() =
        runTest {
            val request =
                HttpConsumerRegistrationRequest(
                    callbackUrl = "https://example.com/webhook",
