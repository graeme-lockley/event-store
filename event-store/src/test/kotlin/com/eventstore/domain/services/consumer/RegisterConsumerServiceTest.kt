...
class RegisterConsumerServiceTest {
    ...
    @Test
    fun `should register consumer successfully`() =
        runTest {
            val request =
    ...
    @Test
    fun `should throw exception for invalid callback URL`() =
        runTest {
    ...
    @Test
    fun `should validate all topics exist`() =
        runTest {
            val nonExistentTopicId = UUID.randomUUID()
            val request =
                HttpConsumerRegistrationRequest(
    ...
    @Test
    fun `should handle multiple topics with lastEventIds`() =
        runTest {
    ...
    @Test
    fun `should throw exception for empty topics map`() =
        runTest {
            val initialConsumers = application.consumerRepository.findAll()
    ...
    @Test
    fun `should allow multiple consumers for same topic`() =
        runTest {
            val request1 =
                HttpConsumerRegistrationRequest(
    ...
    @Test
    fun `should store lastEventId correctly`() =
        runTest {
    ...
    @Test
    fun `should store null lastEventId correctly`() =
        runTest {
            val request =
                HttpConsumerRegistrationRequest(
                    callbackUrl = "https://example.com/webhook",
    ...
    @Test
    fun `should handle consumers for different topics`() =
        runTest {
    ...
    @Test
    fun `should throw exception when invalid UUID provided as topic`() =
        runTest {
            val initialConsumers = application.consumerRepository.findAll()
            ...
            val request =
                HttpConsumerRegistrationRequest(
                    callbackUrl = "https://example.com/webhook",
                    // Valid UUID, but topic doesn't exist
                    topics = mapOf(UUID.randomUUID() to null),
                )
