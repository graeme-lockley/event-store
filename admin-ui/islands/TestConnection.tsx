...
export default function TestConnection({ storeName, url, port }: TestConnectionProps) {
  ...
  const testConnection = async () => {
    ...
    try {
      console.log(`TestConnection: Testing connection to ${storeName} at ${url}:${port}`);
      ...
      if (response.ok && data.success === true) {
        setResult({
          success: true,
          message: `✅ Connection successful! Store is healthy with ${data.consumers || 0} consumers and ${(data.runningDispatchers || []).length} running dispatchers.`,
        });
      } else {
        setResult({
          success: false,
          message: `❌ Connection failed: ${data.error || data.message || "Unknown error"}`,
        });
      }
    } catch (error) {
