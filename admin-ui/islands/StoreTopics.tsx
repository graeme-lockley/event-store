...
import { Topic, HealthStatus } from "../utils/eventStore.ts";
...
export default function StoreTopics({ store }: StoreTopicsProps) {
  ...
  const loadStoreData = async () => {
    ...
    try {
      const res = await fetch(`/api/store-topics?store=${encodeURIComponent(store.name)}`);
      ...
      <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
          ...
          <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">Store Unavailable</h3>
          ...
          <div class="mb-4 p-3 bg-gray-50 rounded-md">
            <div class="flex items-center justify-between text-sm">
              <span class="text-gray-600">
                Consumers: <span class="font-medium">{storeData.health.consumers}</span>
              </span>
              <span class="text-gray-600">
                Dispatchers: <span class="font-medium">{storeData.health.runningDispatchers.length}</span>
              </span>
            </div>
          </div>
        )}
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                {storeData.topics.map((topic) => (
                  <tr key={topic.name} class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-900">{topic.sequence}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      ...
                      <div class="text-xs text-gray-500">
                        {topic.schemas.map(s => s.eventType).join(', ')}
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <a
                        href={`/topics/stats?topic=${encodeURIComponent(topic.name)}&store=${encodeURIComponent(store.name)}`}
                        class="text-blue-600 hover:text-blue-900 mr-4"
                      >
                        View Details
                      </a>
                      <a
                        href={`/events?store=${encodeURIComponent(store.name)}&topic=${encodeURIComponent(topic.name)}`}
