...
export default function StoreList() {
  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      const storesWithHealth: StoreWithHealth[] = storesArray.map((
        store: Store,
      ) => ({
        ...store,
  ...
  const checkStoreHealth = async (store: Store, index: number) => {
    try {
      ...
      if (response.ok) {
            ...
            i === index
              ? {
                ...
                health: {
                  status: "healthy",
                  message:
                    `Healthy - ${consumers} consumers, ${runningDispatchers.length} dispatchers`,
                  consumers: consumers,
        ...
        console.log(
          `StoreList: Health check failed for ${store.name}:`,
          errorData,
        );
          ...
          prev.map((s, i) =>
            i === index
              ? {
                ...
                health: {
                  status: "unhealthy",
                  message: errorData.error || "Store unavailable",
                },
              }
              : s
          )
      ...
      setStores((prev) =>
        prev.map((s, i) =>
          i === index
            ? {
              ...s,
              health: { status: "error", message: "Health check failed" },
            }
            : s
        )
      );
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
            ...
            <div>
              ...
              <div class="mt-1 flex items-center space-x-2">
                ...
                <span
                  class={`text-sm ${
                    store.health?.status === "healthy"
                      ? "text-green-600"
                      : store.health?.status === "unhealthy"
                      ? "text-red-600"
                      : store.health?.status === "error"
                      ? "text-yellow-600"
                      : "text-gray-500"
                  }`}
                >
