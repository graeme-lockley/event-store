...
import { EventStoreClient, Consumer } from "../utils/eventStore.ts";
            ...
            <div class="px-4 py-5 sm:p-6">
              ...
              {store.consumers.length === 0 ? (
                <div class="text-center py-12">
                  ...
                  <h3 class="mt-2 text-sm font-medium text-gray-900">No consumers</h3>
                  <p class="mt-1 text-sm text-gray-500">
                      ...
                      {store.consumers.map((consumer) => (
                        <tr key={consumer.id} class="hover:bg-gray-50">
                          <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{consumer.id}</div>
                          </td>
                          <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{consumer.callback}</div>
                          </td>
                          <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                              {Object.keys(consumer.topics).length} topic{Object.keys(consumer.topics).length !== 1 ? 's' : ''}
                            </div>
                            <div class="text-xs text-gray-500">
                              {Object.entries(consumer.topics).map(([topic, lastEventId]) => (
                                <div key={topic}>
