...
import { EventStoreClient, Event } from "../utils/eventStore.ts";
      ...
      try {
        ...
        if (selectedStore === storeConfig.name && selectedTopic) {
          try {
            ...
            console.error(`Failed to get events for topic ${selectedTopic}:`, error);
          }
        }
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => window.location.href = '?store=' + e.currentTarget.value}
              >
            ...
            <div>
              ...
              <select
                ...
                name="topic"
                class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                onChange={(e) => window.location.href = `?store=${data.selectedStore}&topic=${e.currentTarget.value}`}
              >
                <option value="">All Topics</option>
                {data.selectedStore && data.stores.find(s => s.config.name === data.selectedStore)?.topics.map((topic) => (
                  <option 
                    value={topic}
            ...
            <div>
              ...
              <select
                ...
                name="limit"
                class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                onChange={(e) => window.location.href = `?store=${data.selectedStore}&topic=${data.selectedTopic}&limit=${e.currentTarget.value}`}
              >
            ...
            <div class="px-4 py-5 sm:p-6">
              <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-sm text-gray-500">
                  {data.stores.find(s => s.config.name === data.selectedStore)?.events.length || 0} events
                </span>
              </div>

              {data.stores.find(s => s.config.name === data.selectedStore)?.events.length === 0 ? (
                <div class="text-center py-12">
                  ...
                  <p class="mt-1 text-sm text-gray-500">
                    No events found in this topic. Try selecting a different topic or store.
                  </p>
                </div>
              ) : (
                <div class="space-y-4">
                  {data.stores.find(s => s.config.name === data.selectedStore)?.events.map((event) => (
                    <div key={event.id} class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
          ...
          <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
              <div class="text-center py-12">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">Select a store and topic</h3>
                <p class="mt-1 text-sm text-gray-500">
                  Choose a store and topic from the filters above to view events.
                </p>
              </div>
            </div>
          </div>
