...
export const handler: Handlers = {
  async POST(req) {
    try {
      ...
      try {
        console.log(
          `TestConnection API: Testing connection to ${storeConfig.name} at ${storeConfig.url}:${storeConfig.port}`,
        );

        // Use test-optimized client for faster response times
        const client = createTestEventStoreClient(
          storeConfig as LegacyEventStoreConfig,
        );
        ...
        return new Response(
          JSON.stringify({
            success: true,
            message:
              `✅ Connection successful! Store is healthy with ${health.consumers} consumers and ${health.runningDispatchers.length} running dispatchers.`,
